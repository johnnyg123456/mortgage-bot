...
const { LOAN_NUMBER_PROPERTY } = require('./notion-schema');
...
async function createLoanFromApproval({ borrowerName, loanNumber }) {
  const loanId = normalizeLoanNumber(loanNumber);
  const title = borrowerName?.trim() || (loanId ? `Loan ${loanId}` : 'Unknown Borrower');

  return createPage(LOANS_DB, {
...
async function findOrCreateLoanFromApproval({ borrowerName, loanNumber }) {
  const loanId = normalizeLoanNumber(loanNumber);
  ...
  const loan = await createLoanFromApproval({ borrowerName, loanNumber: loanId });
