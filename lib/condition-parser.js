...
function loadCorrections() {
  try {
    ...
    // Must match where correction-handler saves — /tmp on Vercel, local path in dev
...
async function findLoanByBorrowerOrNumber(borrowerName, loanNumber) {
  ...
  if (borrowerName) {
    // Try full name first, then first+last separately to avoid false matches
    ...
    if (parts.length >= 2) {
      // Try last name (more selective than first name)
    ...
    // First-name-only fallback — only use if exactly one match
