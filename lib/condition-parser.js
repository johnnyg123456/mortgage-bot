...
const UWM_CATEGORIES = [
  'Closing Disclosure', 'HOMEOWNERS INSURANCE', 'Appraisal (Conv)', 'Appraisal',
  'Closing', 'Income', 'Purchase', 'Invoice', 'Credit', 'Title', 'HOI', 'REO', 'TC'
];
...
function parseUwmApprovalLetter(text) {
  ...
  const ptd = conditions.filter(c => !isPtfCondition({ section: c.section, category: c.category, text: c.text }));
  ...
  return {
    ...
    conditions: ptd.map(c => ({
      ...
      category:     'PTD',
      section:      c.section,
      text:         formatConditionTitle({ code: 'PTD', text: c.text }),
      needs_review: false
    }))
  };
}
...
async function handle({ subject, from, body, pdfBuffer, msgId, gmail, threadId, inboxLabel }) {
  ...
  if (!parsed) {
    ...
    if (parsed) log('arc-parsed', { borrower: parsed.borrower_name, loan: parsed.loan_number, count: parsed.conditions.length });
  }

  if (!parsed) {
    const { parseOpenConditionsApproval } = require('./open-conditions-parser');
    parsed = parseOpenConditionsApproval(text);
    if (parsed) log('open-conditions-parsed', { borrower: parsed.borrower_name, loan: parsed.loan_number, count: parsed.conditions.length });
  }

  if (!parsed) {
    const { parseCakeApprovalLetter } = require('./cake-parser');
    parsed = parseCakeApprovalLetter(text);
    if (parsed) log('cake-parsed', { borrower: parsed.borrower_name, loan: parsed.loan_number, count: parsed.conditions.length });
  }

  if (!parsed) {
    const { parseAmwestApprovalLetter } = require('./amwest-parser');
    parsed = parseAmwestApprovalLetter(text);
    if (parsed) log('amwest-parsed', { borrower: parsed.borrower_name, loan: parsed.loan_number, count: parsed.conditions.length });
  }

  if (!parsed) {
    const { parseTheloanstoreApprovalLetter } = require('./theloanstore-parser');
    parsed = parseTheloanstoreApprovalLetter(text);
    if (parsed) log('theloanstore-parsed', { borrower: parsed.borrower_name, loan: parsed.loan_number, count: parsed.conditions.length });
  }

  if (!parsed) {
    const { parseThelenderApprovalLetter } = require('./thelender-parser');
    parsed = parseThelenderApprovalLetter(text);
    if (parsed) log('thelender-parsed', { borrower: parsed.borrower_name, loan: parsed.loan_number, count: parsed.conditions.length });
  }

  if (!parsed) {
    const { parseBluepointApprovalLetter } = require('./bluepoint-parser');
    parsed = parseBluepointApprovalLetter(text);
    if (parsed) log('bluepoint-parsed', { borrower: parsed.borrower_name, loan: parsed.loan_number, count: parsed.conditions.length });
  }

  if (!parsed) {
    const { parseFundloansApprovalLetter } = require('./fundloans-parser');
    parsed = parseFundloansApprovalLetter(text);
    if (parsed) log('fundloans-parsed', { borrower: parsed.borrower_name, loan: parsed.loan_number, count: parsed.conditions.length });
  }

  if (!parsed) {
    const { parseOrionApprovalLetter } = require('./orion-parser');
    parsed = parseOrionApprovalLetter(text);
    if (parsed) log('orion-parsed', { borrower: parsed.borrower_name, loan: parsed.loan_number, count: parsed.conditions.length });
  }
  ...
  const { loan, created } = await findOrCreateLoanFromApproval({ borrowerName, loanNumber });
  ...
  if (pdfBuffer) {
    ...
    const result = syncConditions.length
      ? await syncConditionsToNotion(loan.id, syncConditions)
      : { added: 0, cleared: 0, unchanged: 0, total: 0, addedList: [], clearedList: [] };
