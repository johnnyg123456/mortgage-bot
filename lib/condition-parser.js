...
const PTF_SECTION_RE = /prior\s+to\s+funding|\(ptf\)|\bptf\b/i;
...
function isPtfCondition(cond) {
  ...
  // Untagged lender PDFs (e.g. AmWest) — common closing/PTF-only items
...
function isSectionHeader(line) {
  ...
  return /^(master|uw[\s\-]*prior|underwriter|closing\b|prior\s+to\s+funding)/i.test(line)
...
async function clearPtfConditionsForLoan(borrowerRef) {
  const loan = await findLoanByBorrowerOrNumber(borrowerRef, null);
  if (!loan) throw new Error(`Loan not found for: ${borrowerRef}`);
