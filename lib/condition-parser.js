...
async function syncConditionsToNotion(loanPageId, pdfConditions, meta = {}) {
  const existing = await getOpenConditionsForLoan(loanPageId);
  ...
  for (const pdfCond of pdfConditions) {
    ...
    if (match) {
      ...
      const wasAdded = await addConditionToNotion(
        loanPageId, pdfCond.text, pdfCond.needs_review ?? false, meta, { skipDuplicateCheck: true }
      );
      if (wasAdded) {
        added++;
  ...
  // Only ADD missing conditions and leave existing open items alone.
  // Clears happen only when the PDF explicitly marks a condition cleared
  // (e.g. Acra DATE CLEARED) via clearMatchedConditions — not because the
  // parser missed an item on this pass.

  return { added, cleared: 0, unchanged, total: pdfConditions.length, addedList, clearedList: [] };
}
...
async function handle({ subject, from, body, pdfBuffer, msgId, gmail, threadId, inboxLabel }) {
  ...
  if (!parsed) {
    if (!looksLikeApprovalLetter(text)) {
      log('skip', { reason: 'not an approval letter pdf' });
      return;
    }
    try {
  ...
  if (pdfBuffer) {
    ...
    const result = syncConditions.length
      ? await syncConditionsToNotion(loan.id, syncConditions, conditionMeta)
