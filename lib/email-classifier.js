...
const LENDER_REQUEST_KEYWORDS = [
  ...
  'as soon as possible', 'by end of day', 'by eod'
];
...
function extractLoanHintFromSubject(subject) {
  ...
  if (embedded) {
    const borrower = trimmed.replace(embedded[0], '').replace(/[-–]/g, ' ').trim() || null;
    return { borrower, loanNumber: embedded[1] };
  }
  return {};
}
...
function isLenderUrgentRequest(subject, body, from) {
  const { loanNumber } = extractLoanHintFromSubject(subject);
  if (!loanNumber || !isDirectLenderEmail(from)) return false;

  const text = `${subject} ${body}`.toLowerCase();
  const hasRole    = LENDER_ROLE_KEYWORDS.some(k => text.includes(k));
  const hasRequest = LENDER_REQUEST_KEYWORDS.some(k => text.includes(k))
    || /\b(today|asap)\b/i.test(text);

  return hasRole || hasRequest;
}
