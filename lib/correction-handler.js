...
async function sendConfirmationEmail(summary) {
  ...
  const subject = '[Mortgage Bot] Correction Applied';
  const bodyText = `A correction has been applied to Notion.\n\n${summary}`;
  ...
  for (const account of Object.values(clients)) {
    try {
      ...
      log('email-sent', { inbox: account.label });
    } catch (err) {
      log('email-error', { inbox: account.label, error: err.message });
    }
  }
}
