...
async function handle({ subject, from, body }) {
  ...
  try {
    ...
    if (properties.Priority && /priority/i.test(err.message)) {
      log('priority-field-error', { error: err.message });
  ...
  log('notion-created', { conditionTitle, loanLinked: !!loan, priority: 'High' });
