...
const { findLoanByNumberOrBorrower } = require('./loan-service');
const {
  extractBorrowerLastName,
  detectLenderName,
  buildConditionProperties
} = require('./condition-fields');
const { isTitleOrClosingEmail } = require('./email-parties');
...
async function handle({ subject, from, body }) {
  ...
  if (isTitleOrClosingEmail(from, subject, body)) {
    log('skipped-title-email', { subject });
    return;
  }
  ...
  const loan = await findLoanByNumberOrBorrower(loanNumber, borrower);
  ...
  const lenderName = detectLenderName({ from, subject, pdfText: body })
    || loan?.properties?.Lender?.rich_text?.[0]?.plain_text
    || null;
  const borrowerLastName = extractBorrowerLastName(borrower);

  const properties = buildConditionProperties({
    conditionText: conditionTitle,
    loanPageId: loan?.id,
    needsReview: false,
    borrowerLastName,
    lenderName,
